...
async def userinfo(ctx, user: Option(discord.Member, "Money of which person?", required=False, default=None)):
    ...
    embed.add_field(name="__Money:__", value=f"{getMoney(user.id)} **{currency}**")
...
async def addmoney(ctx, moneytoadd, user: Option(discord.Member, "Money of which person?", required=False, default=None)):
    if user is None:
        user = ctx.author
    if ctx.author.guild_permissions.administrator:
        print(user.id)
        addMoney(int(user.id), moneytoadd)
